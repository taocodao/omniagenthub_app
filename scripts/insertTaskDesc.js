...
import { Pinecone } from '@pinecone-database/pinecone'; // Maintaining your import
...
const pinecone = new Pinecone({
    ...
    //environment: process.env.PINECONE_ENVIRONMENT, // Ensure this is set in your .env file
});
...
const storeEmbedding = async (embedding, metadata) => {
    ...
    const namespace = ""; // Specify your namespace here if not using the default

    try {
        // Check if the vector already exists in Pinecone
        const fetched = await index.fetch([metadata.id], namespace);

        // Log the fetched result to debug
        console.log(`[${new Date().toISOString()}] Fetch result for vector ID "${metadata.id}":`, JSON.stringify(fetched, null, 2));

        // Check if fetched vectors contain the metadata ID
        if (fetched.vectors && fetched.vectors[metadata.id]) {
            console.log(`[${new Date().toISOString()}] Vector with ID "${metadata.id}" already exists. Skipping upsert.`);
            return; // Exit the function to prevent duplicate insertion
        }

        // Proceed to upsert if the vector does not exist
        const vectors = [
            {
        ...
        console.log(`[${new Date().toISOString()}] Successfully upserted vector with ID "${metadata.id}".`);
    } catch (error) {
        console.error(`[${new Date().toISOString()}] Error in storeEmbedding for ID "${metadata.id}":`, error.message);
...
// Function to process each task with retry mechanism
const processTask = async (department, role, task, retries = 3, delay = 1000) => {
    const description = await fetchTaskDescription(department, role, task, 'English', retries);
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            // Log the entire embedding response for debugging
            console.log(`[${new Date().toISOString()}] Embedding response for task "${task}":`, JSON.stringify(embeddingResponse, null, 2));

            // Check if the API returned an error
            if (embeddingResponse.error) {
                console.error(`[${new Date().toISOString()}] OpenAI API Error for task "${task}":`, embeddingResponse.error);
                throw new Error(`OpenAI API Error: ${embeddingResponse.error.message}`);
            }
            ...
            console.error(`[${new Date().toISOString()}] Error generating embedding for task "${task}" on attempt ${attempt}:`, error);

            // Check if it's a rate limit error
            if (error.response && error.response.status === 429 && attempt < retries) {
                const backoff = delay * Math.pow(2, attempt - 1);
                console.log(`[${new Date().toISOString()}] Rate limit hit. Waiting ${backoff}ms before retrying...`);
...
const main = async () => {
    ...
    // Parse command-line arguments as departments
    const args = process.argv.slice(2);
    const departments = args;

    if (departments.length === 0) {
        console.error(`[${new Date().toISOString()}] No departments provided as input. Please specify one or more departments.`);
        process.exit(1);
    }

    console.log(`[${new Date().toISOString()}] Processing ${departments.length} department(s):`, departments);
    ...
    for (const department of departments) {
        ...
        for (const role of roles) {
            ...
            console.log(`[${new Date().toISOString()}] Found ${tasks.length} task(s) in role "${role}" of department "${department}":`, tasks);

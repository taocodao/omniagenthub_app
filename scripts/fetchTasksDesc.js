...
const getTasks = async (department, role) => {
    ...
    try {
        ...
        if (response.status === 200) {
            console.log(`[${new Date().toISOString()}] Tasks fetched for role "${role}" in department "${department}":`, response.data.tasks);
...
const fetchTaskDescription = async (department, role, task, language = 'English', retries = 3) => {
    for (let attempt = 1; attempt <= retries; attempt++) {
        console.log(`[${new Date().toISOString()}] Attempt ${attempt}: Fetching description for task "${task}" in role "${role}" of department "${department}" with language "${language}"...`);
        try {
            ...
            if (response.status === 200 && response.data.description) {
                console.log(`[${new Date().toISOString()}] Successfully fetched description for task "${task}" in role "${role}" of department "${department}".`);
                return response.data.description;
            } else {
                console.warn(`[${new Date().toISOString()}] Attempt ${attempt}: Description missing or empty for task "${task}" in role "${role}" of department "${department}".`);
            }
        } catch (error) {
            console.error(`[${new Date().toISOString()}] Attempt ${attempt}: Error fetching description for task "${task}" in role "${role}" of department "${department}":`, error.message);
        }
    ...
    console.error(`[${new Date().toISOString()}] Failed to fetch description for task "${task}" in role "${role}" of department "${department}" after ${retries} attempts.`);
    return null;
};
...
const main = async () => {
    ...
    for (const department of departmentsToProcess) {
        ...
        for (const role of roles) {
            ...
            for (const task of tasks) {
                ...
                if (!description) {
                    // If fetching description failed or description is empty, add to failedTasks
    ...
    if (failedTasks.length > 0) {
        ...
        console.error(`[${new Date().toISOString()}] Fetched descriptions for some tasks failed. Check 'failedTasks.json' for details.`);
    } else {
        console.log(`[${new Date().toISOString()}] All task descriptions fetched successfully.`);
    }

    console.log(`[${new Date().toISOString()}] Task description fetching process completed.`);
};

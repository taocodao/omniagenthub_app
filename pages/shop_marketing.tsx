...
const Shop: React.FC<ShopProps> = ({
    ...
    useEffect(() => {
        const handleRoleSelection = (roleData: RoleEventData) => {
            ...
            setTimeout(async () => {
                const localizedDepartment = await getLocalizedString(processedRoleData.department, language);
                const localizedRole = await getLocalizedString(processedRoleData.role, language);

                /*eventBus.emit(EVENT_TYPES.EXPAND_PANE_UPDATE, {
                    breadcrumb: `${localizedDepartment} → ${localizedRole}`,
    ...
    const [showExpandPane, setShowExpandPane] = useState(false);
    ...
    // ADD: State to store role summaries
    //const [roleSummaries, setRoleSummaries] = useState<Map<string, string>>(new Map());
    //const [summariesLoaded, setSummariesLoaded] = useState<boolean>(false);

    // New efficient state management with English-first strategy
    ...
    const formatResults = (results: any[]) => {
        ...
        const formattedResults = results.map((result, index) => {
            ...
            const url = `${baseUrl}/ChatHome_new?selectedRole=${encodedRole}&selectedCategory=${encodedDepartment}&selectedTask=${encodedTask}`;
    ...
    const handleRoleClick = (role: RoleMapping) => {
        ...
        if (window.setSelectedRole && role.role && role.role.trim() !== '') {
            window.setSelectedRole(role.role);
        } else {
            console.warn('Shop: Attempted to set empty role, skipping call');
        }
    ...
    const fetchFavoritedRoleMappings = async () => {
        ...
        try {
            const response = await fetch('/api/get-role-by-address_mkt', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    useEffect(() => {
        // Strict check: if role came from URL, NEVER reset it here
        if (isRoleFromURL) {
        ...
        // Fix: If the category change matches the current role's department, DO NOT reset
        // This handles the case where selecting a role also updates the category
        if (selectedRole && selectedRole.department === currentCategory) {
            console.log(`[CategoryChange] Category changed onto active role's department '${currentCategory}'. SKIPPING reset.`);
            return;
        }

        // Only reset if we are sure it's a manual category change and NOT from URL
        console.log(`[CategoryChange] Category changed by user to '${currentCategory}'. Resetting selectedRoleName.`);
        setSelectedRoleName('');
        if (window.setSelectedRole) {
            window.setSelectedRole('');
        }
    }, [currentCategory, isRoleFromURL, selectedRole]); // Add selectedRole to dependencies
    ...
    useEffect(() => {
        const handleExpandPaneUpdate = (data: { breadcrumb: string, role: RoleEventData }) => {
            ...
            setBreadcrumb(data.breadcrumb); // You need to define this state
            setEventSelectedRole(data.role); // Already used
        };
    ...
    const handleRoleToggle = async (role: string, department: string) => {
        ...
        const endpoint = isSelected ? '/api/remove-role-mapping_mkt' : '/api/add-role-mapping_mkt';
        try {

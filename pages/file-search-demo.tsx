...
export default function FileSearchDemo(): JSX.Element {
  ...
  // Get userKey from URL or localStorage
  useEffect(() => {
    if (!router.isReady) return;

    const urlUserKey = router.query.userKey as string;
    if (urlUserKey) {
      setWalletAddress(urlUserKey);
      localStorage.setItem('kb_userKey', urlUserKey);
    } else {
      const storedKey = localStorage.getItem('kb_userKey');
      if (storedKey) setWalletAddress(storedKey);
    }
    setIsReady(true);
  ...
  useEffect(() => {
    if (walletAddress) checkServerHealth();
  }, [walletAddress]);
  ...
  const addSelectedDriveFiles = async () => {
    ...
    for (const file of selectedDriveFiles) {
      try {
        await fetch(`${MCP_ENDPOINT}/tools/add_drive_source`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'X-Wallet-Address': walletAddress },
  ...
  useEffect(() => {
    if (!router.isReady || !walletAddress) return;

    const connected = router.query.connected as string;
    if (connected === 'github') {
      setShowAddModal(true);
      setAddType('github');
      fetchGithubRepos();
      // Clean URL but preserve userKey
      const newUrl = router.pathname + (router.query.userKey ? `?userKey=${router.query.userKey}` : '');
  ...
  }, [router.isReady, router.query.connected, walletAddress]);

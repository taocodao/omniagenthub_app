...
export default async function handler(
    ...
    try {
        ...
        const companyKey = `companyName1:${userAddress}`;
        ...
        if (!Array.isArray(storedUserAddresses)) {
            return res.status(200).json({ users: [] });
        }

        // Exclude the requesting user.
        const matchingUserAddresses = (storedUserAddresses as string[]).filter(
            (addr) => addr !== userAddress
        );
        console.log("[getCompanyUsers] Matching user addresses:", matchingUserAddresses);

        // Retrieve each matching user's name.
        const companyUsers = await Promise.all(
            matchingUserAddresses.map(async (addr) => {
                try {
                    const userName = await kv.get(`userName:${addr}`);
                    return userName ? { userAddress: addr, userName } : null;
                } catch (error) {
                    console.error(`Error retrieving userName for ${addr}:`, error);
                    return null;
                }
            })
        );

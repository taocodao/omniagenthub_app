...
export default async function handler(
    ...
    try {
        ...
        res.status(200).json({ selectedSources: updated });
    } catch (error) {
        console.error("Error removing selected source:", error);
        res.status(500).json({ error: "Error removing selected source" });
    }
}

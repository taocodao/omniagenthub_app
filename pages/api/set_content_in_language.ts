...
async function translateDescription(name: string, description: string, language: string): Promise<string> {
    // If the target language is English, return the original content without translation
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        for (const language of languages) {
            const translatedContent = await translateDescription(name, content, language);
            translations[language] = translatedContent;
            await kv.set(`content:${name}:${language}`, translatedContent);
        }

        res.status(200).json({ message: 'Content translated and saved', translations });
    } catch (error) {

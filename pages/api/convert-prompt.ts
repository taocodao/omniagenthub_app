...
const parseJsonObject = (jsonString: string) => {
    try {
        ...
        console.error('JSON parsing error:', e, 'Invalid JSON:', jsonString); // Log the error and the JSON string
        return {};
    }
};
...
const cleanPrompt = (prompt: string) => {
    return prompt
        .replace(
            /📌📌 \[Subscribe to the AI for Work Newsletter 🤖\]\([^\)]+\)\. AI in marketing is advancing so fast - you cannot afford to get left behind at this stage in your career\. Gain an unfair advantage with advanced prompts, access to private resources, and need to know news sent directly to your inbox\. \[Subscribe Now!\]\([^\)]+\) 📌📌/g,
            ''
        )
        .replace(/aiforwork\.co/g, '');
};
...
const parsePrompt = (prompt: string) => {
    ...
    const newRules = {
        rule_1:
            "If user starts with 'Ask 5 questions,' do NOT ask questions. Instead, immediately make educated assumptions for any missing information based on the context and proceed to execute the task fully. State your assumptions briefly at the beginning.",
        rule_2:
        ...
        if user input starts with "refine":
            ...
            Append a "CHANGE LOG 📝" section (as per rule 9) documenting specific alterations and updates.
    ...
    const structuredPrompt = {
        ...
        rules: newRules,
        key_references: keyReferencesMatch
            ? parseJsonObject(`{${keyReferencesMatch[1]}}`)
            : {},
        criteria: criteriaMatch ? parseJsonObject(criteriaMatch[1]) : {},
        evaluationRubric: evaluationRubricMatch
            ? parseJsonObject(`{${evaluationRubricMatch[1]}}`)
            : {},
    };

    console.log('Structured Prompt:', JSON.stringify(structuredPrompt, null, 2));
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!prompt || !role || !task || !department) {
        return res.status(400).json({
            message: 'Department, Role, Task, and Prompt are required',
        });
    }

    const structuredPrompt = parsePrompt(prompt);
    //const key = HashUtil.hashTo(`${department}:${role}:${task}`);
    ...
    // Helper function to parse data from KV
    ...
    try {
        // Fetch roles and tasks from KV store
        const rolesData = await kv.get(departmentRolesKey);
        const tasksData = await kv.get(roleTasksKey);

        // Parse the roles and tasks data
        const roles: any[] = parseData(rolesData);
        const tasks: any[] = parseData(tasksData);
        ...
        if (!rolesSet.has(role)) {
            ...
            await kv.set(departmentRolesKey, JSON.stringify(rolesArray));
            console.log('Roles after adding new role:', rolesArray);
        }

        // Add the new task if it doesn't exist
        if (!tasksSet.has(task)) {
            tasksSet.add(task);
            const tasksArray = Array.from(tasksSet);
            await kv.set(roleTasksKey, JSON.stringify(tasksArray));
            console.log('Tasks after adding new task:', tasksArray);
        }

        // Save the instructions associated with the task
        ...
        return res
            .status(500)
            .json({ message: 'Error saving prompt to KV database.' });
    }
}

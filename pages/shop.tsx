...
const Shop = () => {
    ...
    const handleSetCompanyName = async () => {
        ...
        const isDuplicate = companyNames.some(
    ...
    const formatResults = (results: any[]) => {
        ...
        const formattedResults = results.map((result, index) => {
            ...
            const url = `${baseUrl}/ChatHome?selectedRole=${encodedRole}&selectedCategory=${encodedDepartment}&selectedTask=${encodedTask}`;
    ...
    return (
        <main className={styles.main}>
                    ...
                    <>
                        ...
                        <div className={styles.headerInfo}>
                            {nameData.companyName ? (
                                <div>
                                    <LocalizedText name="Company" />: {nameData.companyName}
                                </div>
                            ) : null}
                            {nameData.userName ? (
                                <div>
                                    <LocalizedText name="User" />: {nameData.userName}
                                </div>
                            ) : null}
                        </div>
                        ...
                        {showCompanyModal && (
                            <div className={styles.subModalOverlay}>
                                <div className={styles.subModalContent}>
                                    ...
                                    {companyNames.length > 0 && (
                                        <div style={{ marginTop: "1rem" }}>
                                            ...
                                            <select
                                                value={tempCompanyName}
                                                onChange={(e) => setTempCompanyName(e.target.value)}
                                                style={{ width: "100%", padding: "0.5rem", fontSize: "16px", borderRadius: "8px" }}
                                            >
                                    ...
                                    <button onClick={handleSetCompanyName} className={styles.subModalButton} style={{ marginTop: "1rem" }}>
                        ...
                        <div className={styles.categorySection}>
                            <div className={styles.categorySectionInner}>
                                ...
                                >
                                    ...
                                    {categories
                                        .filter((c) => c !== 'Favorited' && c !== 'My Customized GPT')
                                        .map((category) => (
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        // Reset messages and input when opening the modal
                                        setMessages([]);
                                        setInput({
                                            query: '',
                                        });
                            ...
                            {roles.length > 0 ? (
                                roles.map(({ department, role }) => {
                                    ...
                                    const currentMapping =
                                        roleMappings[department]?.find((mapping) => mapping.role === role) ||
                                    ...
                                    return (
                                        <div key={roleKey} className={styles.nftContainer}>
                                            <div className={`${styles.nftImageWrapper} ${isSelected ? styles.selected : ''}`}>
                                                {/* 1. Checkbox Overlay */}
                                                <div className={styles.tooltipContainer}>
                                                    ...
                                                    <span className={styles.tooltipText}>
                                                        {isSelected ? (
                                                            <LocalizedText name="Uncheck to deselect this role" />
                                                        ) : (
                                                            <LocalizedText name="Check to select this role" />
                                                        )}
                                                    </span>
                                                </div>
                        ...
                        {isChatModalOpen && (
                            <div className={styles.modalOverlay}>
                                <div className={styles.modalContent}>
                                    {/* Close Button */}
                                    <button className={styles.closeButton} onClick={() => setIsChatModalOpen(false)}>
                                        X
                                    </button>
                                    {/* Department and Role Selectors */}
                                    <div className={styles.chatSelectors}>
                                        <label className={styles.selectorLabel}>
                                            <LocalizedText name="Department" />:
                                            <select
                                    ...
                                    <div className={styles.chatContainer}>
                                        ...
                                        <form onSubmit={handleSubmit} className={styles.chatInputForm}>
                                            <input
                                                ...
                                                placeholder="What is your primary objective for this task?"
            ...
            {isRoleModalOpen && (
                <div className={styles.modalOverlay}>
                    <div className={styles.modalContent} style={{ top: roleModalPosition.top, left: roleModalPosition.left, backgroundColor: 'blue' }}>
                        <button className={styles.closeButton} onClick={closeRoleModal}>
                            X
                        </button>
                        {/* Center the currentRole in the modal */}
                        <h2 className={styles.modalTitle} style={{ color: 'white', textAlign: 'center', marginBottom: '15px' }}>

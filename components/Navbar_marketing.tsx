...
import { usePrivy, useWallets } from '@privy-io/react-auth'; // ✅ Migrated from Auth0 to Privy
...
const erc20Abi = [
    ...
    "function transfer(address to, uint256 amount) returns (bool)", // Added transfer
];
...
const Navbar: React.FC<NavbarProps> = ({ isMarketingPage = false }) => {
    ...
    const [freeChatsLeft, setFreeChatsLeft] = useState<number | null>(0);
    ...
    const [webaiBalance, setWebaiBalance] = useState<string | null>(null);
    ...
    const [selectedToken, setSelectedToken] = useState<'USD' | 'MATIC'>('USD');
    ...
    const [selectedPerk, setSelectedPerk] = useState<'freeTrades' | 'freeUploads' | 'freeWebScrape' | 'usdc' | ''>('');
    ...
    // Add new state for entrepreneur group departments
    const [entrepreneurGroupDepartments, setEntrepreneurGroupDepartments] = useState<string[]>([]);

    // Dynamic categories list that combines Favorite with entrepreneur group departments
    const categories = useMemo(() => {
        return ['Favorite', ...entrepreneurGroupDepartments];
    }, [entrepreneurGroupDepartments]);

    const { openChat } = isMarketingPage ? businessChatContext : regularChatContext;

    // NEW: Function to fetch entrepreneur group departments
    const fetchEntrepreneurGroupDepartments = useCallback(async () => {
        try {
            ...
            if (response.ok) {
                ...
                console.error('Failed to fetch entrepreneur group departments');
                setEntrepreneurGroupDepartments([]);
            }
        } catch (error) {
    ...
    const handleRoleSelection = (roleName: string) => {
        ...
        const url = `${baseUrl}/ChatHome_new?selectedRole=${encodedRole}&selectedCategory=${encodedCategory}`;
    ...
    // NEW: Fetch entrepreneur group departments on component mount
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        window.addEventListener('refreshWebaiCredits', handleRefreshFreeChats); // Auto-refresh after MCP payment
        ...
        return () => {
            ...
            window.removeEventListener('refreshWebaiCredits', handleRefreshFreeChats);
        };
                ...
                >
                    ...
                    {freeChatsLeft !== null && (
                        <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
                            ...
                            {/* Balance Display - Shows both USD and USDC */}
            ...
            <div style={{
                ...
                <div style={{ display: "flex", gap: "1rem" }}>
                    ...
                    {currentPath === "/ChatHome_bus" ? (
                        <span className={styles.activeNavLink}>
                            <LocalizedText name="Business" />
                        </span>
                    ) : (
                        <Link href="/ChatHome_bus" className={styles.navLink}>
                            <LocalizedText name="Business" />
                        </Link>
                    )}
